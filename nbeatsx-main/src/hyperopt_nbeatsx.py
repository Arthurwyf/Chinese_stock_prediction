...
import sys
sys.path.append("/home/arthur/Time_Series_Forecasting/nbeatsx-main")
...
def get_experiment_space(args):
    ...
    if args.space == 'nbeats_x':
        space = {'initialization':  hp.choice('initialization', ['orthogonal', 'he_normal', 'glorot_normal']),
                ...
                'n_iterations': hp.choice('n_iterations', [10_000]),
                'batch_size': hp.choice('batch_size', [1024, 4096]), # [256, 512]
...
def main(args):
    ...
    test_date = TEST_DATE_N[args.dataset]
    Y_df, X_df, _ = EPF.load_groups_n(directory='./data', groups=[args.dataset])
    
    # Remove test set
    test_date = TEST_DATE_N[args.dataset]
    y_insample_df = Y_df[Y_df['unique_id']!=test_date].reset_index(drop=True)
    X_t_insample_df = X_df[X_df['unique_id']!=test_date].reset_index(drop=True)
    y_outsample_df = Y_df[Y_df['unique_id']==test_date].reset_index(drop=True)
    X_t_outsample_df = X_df[X_df['unique_id']==test_date].reset_index(drop=True)
